<template>
    <AdminLayoutComponent>
        <template #title>وضعیت‌های پروژه</template>
            ...
            <div class="columns is-multiline">
                <div class="column is-12 is-flex align-items-center flex-wrap">
                    <button @click="newItem" v-if="permission.add" class="button is-primary font-weight-bolder mr-2 is-flex align-items-center" :disabled="loading || saving">
                        <i class="fal fa-plus mr-2"></i> <span>وضعیت جدید</span>
                    </button>
        ...
        <b-card no-header :no-footer="!newRec && !editing" :body-class="{'p-0': (!newRec && !editing), 'p-5': newRec || editing}">
            ...
            <template #footer-left>
                <button class="button is-success mr-2" @click="saveItem()" :disabled="loading">
                    ...
                    <span class="ml-2">ذخیره وضعیت پروژه</span>
                </button>
...
export default {
    name: "AdminProjectStatusComponent",
    data() {
        return {
            permission: {
            ...
            headers: [
                ...
                {id: 'title', title: 'عنوان وضعیت پروژه', thClass: '', tdClass: '', thStyle: '', tdStyle: '', sortable: true},
                {id: 'description', title: 'توضیحات وضعیت پروژه', thClass: '', tdClass: '', thStyle: '', tdStyle: ''},
            ...
            form_template: {
                title:       {title: 'عنوان وضعیت پروژه', is_required: true, col_desktop: 6, col_widescreen: 6, validator: ['required', 'min:2', 'max:30']},
                ...
                description: {title: 'توضیحات وضعیت پروژه', type: 2, col_desktop: 12, col_widescreen: 12, validator: ['max:150'], description: 'حداکثر 150 حرف'},
            },
            totalRows: 1,
            currentPage: 1,
            perPage: 10,
    ...
    methods: {
        refreshTable() {
            if (!this.loading) {
                this.loading = true
                Requests.getProjectStatusList().then(res => {
                    if (res?.status) {
                        this.statuses = res?.result ?? [];
                    } else {
                        this.$helpers.notify('خطا', res?.message || 'بروز خطا هنگام دریافت فهرست وضعیت پروژه', { type: 'error' })
                    }
                }).catch(err => {
                    console.error(err)
                    this.$helpers.notify('خطای ناشناخته', err?.response?.data?.message || 'بروز خطای ناشناخته هنگام دریافت فهرست وضعیت پروژه', { type: 'error' })
                }).finally(() => {
                    this.loading = false
                })
            }
            ...
            if (!this.loading) {
                this.$refs.form.validateAll().then(res => {
                    if (res) {
                        ...
                        Requests.saveProjectStatus(record).then(res => {
                            if (res?.status) {
                                if (this.editing) {
                                    for (let i = 0; i < this.statuses.length; i++) {
                                        if (this.statuses[i].id === this.editingItem.id) {
                                            this.statuses[i] = res.result
                                            break;
                                        }
                                    }
                                } else {
                                    this.statuses.push(res.result)
                                }
                                this.$helpers.notify(res?.message || 'وضعیت پروژه مورد نظر با موفقیت ذخیره شد')
                                this.newItem()
                                this.newRec = false
                            } else {
                                this.$helpers.notify('خطا', res?.message || 'بروز خطا هنگام ذخیره وضعیت پروژه')
                            }
                        }).catch(err => {
                            console.error(err)
                            this.$helpers.notify('خطای ناشناخته', err?.response?.data?.message || 'بروز خطای هنگام ذخیره وضعیت پروژه')
                        }).finally(res => {
                            this.loading = false
                        })
                    }
        ...
        deleteItem(item_id) {
            this.$swal.fire({
                title: "حذف وضعیت پروژه",
                html: "حذف وضعیت پروژه یک عملیات غیرقابل بازگشت بوده و علاوه بر حذف آن، از تمامی پروژه‌ها (املاک) نیز برداشته خواهد شد.<span class='ml-2 has-text-danger has-text-weight-bold'>وضعیت پروژه مورد نظر حذف گردد؟</span>",
                icon: "warning",
                ...
                if (result.value) {
                    this.saving = true
                    Requests.deleteProjectStatus(item_id).then(res => {
                        if (res?.status) {
                            for (let i = 0; i < this.statuses.length; i++) {
                                if (this.statuses[i].id == item_id) {
                                    this.statuses.splice(i, 1)
                                    break;
                                }
                            }
                            this.$helpers.notify('وضعیت پروژه مورد نظر با موفقیت حذف گردید.')
                        } else {
                            this.$helpers.notify('خطا', res?.message || 'خطای غیرمنتظره! حذف وضعیت پروژه مورد نظر با مشکل مواجه شد...', {
                                type: "error",
                            })
                        }
                    }).catch(err => {
        ...
        setItemAsDefault(id) {
            if (!this.saving && !this.editing) {
                Requests.setProjectStatusAsDefault(id).then(res => {
                    this.saving = true
                    if (res?.status) {
                        ...
                        this.$helpers.notify('وضعیت پروژه مورد نظر با موفقیت به عنوان پیش‌فرض ثبت شد.')
                    } else {
                        this.$helpers.notify('خطا', 'خطا در ثبت وضعیت پروژه به عنوان پیش‌فرض...', {
                            type: "error",
                        })
                    }
                }).catch(err => {
